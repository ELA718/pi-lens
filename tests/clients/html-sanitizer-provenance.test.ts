...
describe("HTML sanitizer provenance", () => {
	...
	it.each([
		...
		"import purifier from 'dompurify'; import { useMemo as memo } from 'react'; const View = function memo(arg) { if (arg) return input; return <div dangerouslySetInnerHTML={{__html: memo(() => purifier.sanitize(input), [])}} />; };",
		"import purifier from 'dompurify'; const View = class Set { constructor(values) { values?.push('onerror'); } render() { const attrs = ['class']; new Set(attrs); return <div dangerouslySetInnerHTML={{__html: purifier.sanitize(input, {ALLOWED_ATTR: attrs})}} />; } };",
		"import purifier from 'dompurify'; const html = purifier.sanitize(input); const View = function html() { return <div dangerouslySetInnerHTML={{__html: html}} />; };",
		"import purifier from 'dompurify'; const view = <div dangerouslySetInnerHTML={{__html: purifier.sanitize(input)}}",
